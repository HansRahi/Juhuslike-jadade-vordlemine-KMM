#Programm sõltuvuse/sõltumatuse saartega VMM-i näite genereerimiseks
...
Z_saared = W %>% mutate(xmin = pmax(W$t - 0.5, 1), xmax = pmin(W$t + 0.5, n))
...
teoreetilised_df = 
  ...
  mutate(p = ifelse(seisund == "0", dgeom(pikkus - 1, 1-p), dgeom(pikkus - 1, 1-q)),
         tyyp = "Teoreetiline") %>% 
  group_by(seisund) %>%
  mutate(p = ifelse(pikkus == 16, 1 - sum(p[pikkus < 16]), p))
...
x_labels = c(as.character(1:15), ">15")
...
jaotused = ggplot(vordlus_df, aes(x = factor(pikkus), y = p, fill = tyyp)) +
  ...
  facet_wrap(~seisund, scales = "free_x", 
             labeller = as_labeller(c("0" = "Nullide saared", "1" = "Ühtede saared"))) +
  scale_x_discrete(labels = x_labels) + 
  scale_fill_manual(values = c("Empiiriline" = "palegreen2", "Teoreetiline" = "cadetblue2")) +
  theme_bw() +
  labs(x = "Saare pikkus",
       y = "Tõenäosus",
       fill = "Jaotus")
